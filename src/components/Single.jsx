...
class Single extends Component {
  ...
  render(){
    ...
    const fullData = <><table className="table">
    <thead>
        <tr>
            ...
            <td>{employee.state}</td>
                    ...
                    <div className='chart'>
                        <table className='table sm'>
                            <tbody>
                        {
                            Object.keys(days).sort((a,b)=>days[a] - days[b]).map((day, ind)=>{
                                const count = days[day]
                                return <tr key={ind}>
                                            <td>{day}</td>
                                            <td>{count}</td>
                    ...
                    <div className='chart'>
                        <PieChart width={500} height={650}>
                            <Pie isAnimationActive={true} data={data} cx={200} cy={150} outerRadius={120} fill="#82ca9d" label/>
                            <Tooltip/>
                        </PieChart>
                    </div>
                    ...
                    <div className='chart-2'>
                        <table className='table sm'>
                            <tbody>
                        {
                            Object.keys(months).sort((a,b)=>days[a] - days[b]).map((month, ind)=>{
                                const count = months[month]
                                return <tr key={ind}>
                                            <td>{month}</td>
                                            <td>{count}</td>
                    ...
                    <div className='chart-2'>
    <BarChart width={400} height={450} data={arr}
    ...
    return (
        <div className='single'>
            <button onClick={this.goBack} type="button" className="btn btn-outline-dark">{`<<< Go Back`}</button>
            {content}
        </div>
    );
  }
